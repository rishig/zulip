...
class RealmAlias(models.Model):
    ...
    subdomains_allowed = models.BooleanField(default=False)
...
def get_realm_by_email_domain(email):
    ...
    if alias is not None:
        ...
        query = query.filter(subdomains_allowed=True)
        while len(domain) > 0:
...
def list_of_domains_for_realm(realm):
    ...
    return list(RealmAlias.objects.filter(realm = realm).values('domain', 'id', 'subdomains_allowed'))
