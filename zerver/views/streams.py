    ...
    if announce and len(created_streams) > 0:
        ...
        if notifications_stream is not None:
            if len(created_streams) > 1:
                stream_names = ", ".join('#**%s**' % (s.name,) for s in created_streams)
                content = _("@_**%s|%d** created the following streams: %s.") % (
                    user_profile.full_name, user_profile.id, stream_names)
            else:
                content = _("@_**%s|%d** created a new stream #**%s**.") % (
                    user_profile.full_name, user_profile.id, created_streams[0].name)
