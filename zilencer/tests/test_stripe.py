...
class StripeTest(ZulipTestCase):
    ...
    def test_upgrade_with_outdated_seat_count(self, mock_create_subscription: mock.Mock,
        ...
        self.assertEqual(RealmAuditLog.objects.filter(event_type=RealmAuditLog.PLAN_UPDATE_QUANTITY)
                         .values_list('extra_data', flat=True).first(), str({'quantity': new_seat_count}))
